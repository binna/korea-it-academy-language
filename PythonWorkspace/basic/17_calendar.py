...
print('    ' * calenderModule.weekDay(year, month, 1), end='')

# 1일부터 달력을 출력할 달의 마지막 날짜까지 반복하여 달력을 출력한다.
for i in range(1, calenderModule.lastDay(year, month) + 1):
    print(' {0:2d} '.format(i), end='')
    # 출력할 날짜(i)가 토요일이면 줄을 바꾼다.
    if calenderModule.weekDay(year, month, i) == 6 and i != calenderModule.lastDay(year, month):
        print()
print('\n' + '=' * 28)
